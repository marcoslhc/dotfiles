...
# These add some convenience commands for cutting, copying, pasting,
# deleting, and indenting Lisp expressions.

# Applies the function f and then reverts the cursor positions
...
atom.commands.add 'atom-text-editor', 'jason:cut-sexp', ->
  ...
  atom.commands.dispatch(
    atom.views.getView(editor), 'lisp-paredit:expand-selection')
...
atom.commands.add 'atom-text-editor', 'jason:indent-top-sexp', ->
  maintainingCursorPosition ->
    ...
    # Work around a lisp paredit bug where it can't indent a range if selected
